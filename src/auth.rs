...
/// Validate *and consume* a challenge.  Returns `true` if the challenge was
/// generated by this server within the last hour.  Each challenge can only be
/// used once; it is removed from the store on success to prevent replay.
fn consume_challenge(challenge: &str) -> bool {
    let mut map = challenges().lock().unwrap();
    let now = Instant::now();
    if let Some(&created) = map.get(challenge) {
        // Always remove—whether valid or expired—so it cannot be retried.
...
///   or been used, together with a valid Ed25519 signature of that challenge.
///
/// For public-key auth the challenge is consumed on success, preventing replay.
pub fn is_authenticated(params: &HashMap<String, String>) -> bool {
    ...
    if let (Some(challenge), Some(token)) = (params.get("challenge"), params.get("token")) {
        // Verify the signature first (no side-effects), then consume the
        // challenge to prevent it from being reused.
        if verify_token(challenge, token) && consume_challenge(challenge) {
            return true;
        }
    }
...
mod tests {
    ...
    fn challenge_roundtrip() {
        ...
        // First use: valid and consumes the challenge.
        assert!(consume_challenge(&c));
    ...
    fn unknown_challenge_rejected() {
        assert!(!consume_challenge("notachallenge"));
    }
}
